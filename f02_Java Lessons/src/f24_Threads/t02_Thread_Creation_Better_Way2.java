...
единственным назначением метода start () является вызов метода run (). 
	...
	Создание потока путем реализации интерфейса Runnable  */
		/* Усовершенствованная версия класса MyThread  Здесь 
1. Поток начинает выполняться сразу же после того, как он создается. Это достигается за счет создания экземпляра объекта типа
Thread в конструкторе класса MyThread. 
2. Нет никакой нужды хранить в объекте типа MyThread имя потока - оно может быть присвоено потоку при его создании. Эту задачу 
позволяет версия конструктора  Thread(Runnable threadOb, String имя) где имя обозначает конкретное имя потока.  */

class MyThread2 implements Runnable {
			//В этой переменной хранится ссылка на nоток
	Thread thrd;			
			// Создать новый поток		 
	public MyThread2(String name) {
			/* Здесь в конструктор класса MyThread2 передается имя потока,
...
public class t02_Thread_Creation_Better_Way2 {
