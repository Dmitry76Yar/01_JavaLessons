...
/*	СДве кучки камней		(Время: 1 сек. Память: 16 Мб Сложность: 43%)
...
public class t2_Sochet_Algor_in_method {
...
//	System.out.println(Arrays.toString(StonesSet));
	...
	int n = 5;
	int StonesSet[] = {1,2,2,1,2};
//	ArrayList<int[]> strff = getSet(StonesSet, 2);
//	
//	for (int i = 0; i <n; i++)  System.out.println(Arrays.toString(strff.get(i)));
	...
	temp = getSet(StonesSet, 5);
	for (int i=0; i<temp.size(); i++) System.out.println(Arrays.toString(temp.get(i)));
